  ...
  for (const [grado, dir] of Object.entries(gradoDir)) {
    ...
    for (const archivo of archivos) {
      ...
      try {
        ...
        // reto: debe verse el título del pda, la síntesis teórica (página 0), y 20 reactivos
        // paginados de 5 en 5 (4 páginas)
        bodyText = await page.innerText('body');
        if (!bodyText.includes(pda.titulo)) throw new Error('reto no muestra el título del PDA');
        if (!bodyText.includes(pda.reto.sintesis)) throw new Error('reto no muestra reto.sintesis en la primera página');

        const reactivos = pda.reto.reactivos;
        if (reactivos.length !== 20) throw new Error(`se esperaban 20 reactivos en el reto, hay ${reactivos.length}`);

        const totalPaginas = Math.ceil(reactivos.length / TAM_PAGINA_RETO);
        if (totalPaginas !== 4) throw new Error(`se esperaban 4 páginas de reto, hay ${totalPaginas}`);

        for (let pagina = 0; pagina < totalPaginas; pagina++) {
          const inicio = pagina * TAM_PAGINA_RETO;
          const fin = Math.min(inicio + TAM_PAGINA_RETO, reactivos.length);

          bodyText = await page.innerText('body');
          const etiquetaEsperada = `Reactivos ${inicio + 1}–${fin} de ${reactivos.length}`;
          if (!bodyText.includes(etiquetaEsperada)) {
            throw new Error(`no se ve la etiqueta de página esperada "${etiquetaEsperada}" en: ${bodyText.slice(0, 200)}`);
          }
          ...
          if (pagina < totalPaginas - 1) {
            await page.click('[data-accion="reto-siguiente"]');
